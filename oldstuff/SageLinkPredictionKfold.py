...
def train(predictor, x, train_edges, train_ids, optimizer, batch_size,device):
    predictor.train()

    train_edges = train_edges[train_ids].to(device)
...
def routine(x, train_edges, test_edges, params, trial, device, yTrain):
  ...
  validForTest = (validForTest, neg_validForTest)
  test_edges = (test_edges, neg_test_edges)
  x = x.to(device)
  skf = StratifiedKFold(n_splits=params["k-fold"])
  for epoch in tqdm(range(1, 1 + epochs),position=0,leave=True):
    f = 0
    for train_ids, valid_ids in skf.split(train_edges,yTrain):
      f+=1
      print("Fold: ",f)
      loss, pos_loss, neg_loss = train(model, x, train_edges, train_ids, optimizer, batch_size,device)
      mlflow.log_metric("Loss",loss,step=epoch)
      mlflow.log_metric("pos-Loss",pos_loss,step=epoch)
      mlflow.log_metric("neg-Loss",neg_loss,step=epoch)
    trainOutput, validOutput, testOutput, validLoss, posValidLoss, negValidLoss = test(model, x, batch_size, trainForTest, validForTest, test_edges)
  ...
  return testOutput["test_BinaryAUROC"]
...
def objective(trial, x, full_edges, trainEdges, testEdges,device, yTrain):
  with mlflow.start_run():
      params = {
          ...
          "k-fold" : trial.suggest_int("k-fold", low=3, high=10)
      }
...
###TODO
# Sollte eigl nich so schwer sein
# 1. erstell hier den trainTest split auf graph.edge_index
# 2. pass die parameter und train und test funktion an mit dem kfold
...
study = optuna.create_study(
    study_name="PrimeKG LinkPrediction with SAGE encoder and additional metrics and kfold",
    direction="minimize",
